...
## 希利創新彙總檔同步與 Push 自動作業（兩支程式流程）

儀表板用的「希利創新娃娃機_每日交易明細.csv」有兩份：
- **數據分析/BI_希利創新/output/**（BI 專案產出；主程式執行結束時會**自動複製**到 Web）
- **Web/數據分析/BI_希利創新/output/**（給雲端部署用）

**兩支程式即可完成同步與發布**（適合排程）：

| 順序 | 程式 | 說明 |
|------|------|------|
| 1 | `python 希利創新娃娃機_交易統計與報表.py`（於 BI_希利創新 目錄） | 產出彙總檔並**自動複製**到 Web 專案內 |
| 2 | `python app.py --push-data`（於 Web 目錄） | 對彙總檔執行 `git add`、`commit`、`push` 到 GitHub |

前提：雲端硬碟下 `數據分析/BI_希利創新` 與 `Web` 同層；Web 已設好 `origin` 且可正常 push。  
備用：若未跑 BI 主程式，仍可在 BI_希利創新 目錄執行 `python 工具_同步彙總檔到Web.py` 手動複製；push 亦可改由 `python app.py --push-data` 在 Web 專案執行。
